...
S21Multiset<Key, Compare, Alloc>::S21Multiset(
  ...
  for (const auto &it : items) {
    Tree<Key>::insert_node(it, {},WITH_DUPLICATE);
  }
}
...
template <typename Key, typename Compare, typename Alloc>
S21Multiset<Key, Compare, Alloc> &S21Multiset<Key, Compare, Alloc>::operator=(
    const S21Multiset &other) {
  if (this != &other) {
    this->clear();
    for (auto it = other.begin(); it != other.end(); ++it) {
      Tree<Key>::insert_node((*it).first, {},WITH_DUPLICATE);
...
template <typename Key, typename Compare, typename Alloc>
typename S21Multiset<Key, Compare, Alloc>::iterator S21Multiset<Key, Compare, Alloc>::insert(
    const value_type &value) {
  auto pair = Tree<Key>::insert_node(value, {},WITH_DUPLICATE);
  return pair.first;
}
...
S21Multiset<Key, Compare, Alloc>::emplace(Args &&...args) {
  ...
  data.push_back(Tree<Key>::insert_node(std::forward<Args>(args)..., {},WITH_DUPLICATE));
  return data;
}
...
//    const std::initializer_list<value_type> &items) {
//  for (const auto &it : items) {
//    Tree<Key, Value, Compare, Alloc>::insert_node(it.first, it.second, WITH_DUPLICATE);
//  }
//}
//
//template <typename Key, typename Value, typename Compare, typename Alloc>
...
//  return Tree<Key, Value, Compare, Alloc>::insert_node(value.first, value.second, WITH_DUPLICATE);
//}
//
//template <typename Key, typename Value, typename Compare, typename Alloc>
//std::pair<typename S21Multiset<Key, Value, Compare, Alloc>::iterator, bool>
...
//                                                const Value &obj) {
//  return Tree<Key, Value, Compare, Alloc>::insert_node(key, obj, WITH_DUPLICATE);
//}
//
//template <typename Key, typename Value, typename Compare, typename Alloc>
//std::pair<typename S21Multiset<Key, Value, Compare, Alloc>::iterator, bool>
...
//                                                          const Value &obj) {
//  auto pr = insert(key, obj, WITH_DUPLICATE);
...
//  auto end_elem = size - 1;
//  for (auto i = 0; i < size; ++i) {
//    auto res = insert(std::pair(it->first, it->second), WITH_DUPLICATE);
//    if (res.second) {
//      other.erase(it);
//    }
//    if (i < end_elem) {
